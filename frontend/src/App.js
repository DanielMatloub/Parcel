...
function PropertyDetails({ details, show, setShow }) {
  return (
    <div style={{ marginTop: "12px" }}>
      ...
      {show && (
        <div style={{ background: "#f9f9f9", borderRadius: "8px", padding: "12px 16px", marginTop: "8px", fontSize: "13px", lineHeight: "1.8" }}>
          {details.assessed_total_value > 0 && <div><span style={{ color: "#888" }}>Assessed value</span><span style={{ float: "right", fontWeight: "600" }}>${details.assessed_total_value.toLocaleString()}</span></div>}
          {details.assessed_land_value > 0 && <div><span style={{ color: "#888" }}>Land value</span><span style={{ float: "right" }}>${details.assessed_land_value.toLocaleString()}</span></div>}
          {details.assessed_improvement_value > 0 && <div><span style={{ color: "#888" }}>Improvement value</span><span style={{ float: "right" }}>${details.assessed_improvement_value.toLocaleString()}</span></div>}
          {details.use_definition && <div><span style={{ color: "#888" }}>Use type</span><span style={{ float: "right" }}>{details.use_definition}</span></div>}
          {details.year_built && details.year_built !== "0" && <div><span style={{ color: "#888" }}>Year built</span><span style={{ float: "right" }}>{details.year_built}</span></div>}
          {details.lot_area && parseFloat(details.lot_area) > 0 && <div><span style={{ color: "#888" }}>Lot area</span><span style={{ float: "right" }}>{parseFloat(details.lot_area).toLocaleString()} sq ft</span></div>}
          {details.stories && parseFloat(details.stories) > 0 && <div><span style={{ color: "#888" }}>Stories</span><span style={{ float: "right" }}>{details.stories}</span></div>}
          {details.neighborhood && <div><span style={{ color: "#888" }}>Neighborhood</span><span style={{ float: "right" }}>{details.neighborhood}</span></div>}
          {details.last_sale_date && <div><span style={{ color: "#888" }}>Last sale</span><span style={{ float: "right" }}>{details.last_sale_date}</span></div>}
          <div style={{ marginTop: "8px", fontSize: "11px", color: "#aaa" }}>Data from SF Assessor-Recorder, {details.data_year}</div>
        </div>
      )}
    </div>
  );
      ...
      {show && (
        <div style={{ marginTop: "8px" }}>
          {permits.length === 0 ? (
            ...
            permits.map((p, i) => (
              <div key={i} style={{ background: "#f9f9f9", borderRadius: "8px", padding: "12px", marginBottom: "8px", fontSize: "13px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                  <span style={{ fontWeight: "600", textTransform: "capitalize" }}>{p.type}</span>
                  <span style={{ padding: "2px 8px", borderRadius: "12px", fontSize: "11px", background: p.status === "complete" ? "#e8f5e9" : p.status === "expired" ? "#fce4ec" : "#fff8e1", color: p.status === "complete" ? "#388e3c" : p.status === "expired" ? "#c62828" : "#f57f17" }}>
                    {p.status}
                  </span>
                </div>
                <div style={{ color: "#555", marginBottom: "4px" }}>{p.description}</div>
                <div style={{ color: "#888", fontSize: "12px" }}>Filed: {p.filed_date}{p.estimated_cost > 0 && ` · Est. cost: $${p.estimated_cost.toLocaleString()}`}</div>
              </div>
            ))
          )}
        </div>
...
function EnvironmentalRisks({ risks }) {
  ...
  return (
    <div style={{ marginTop: "12px" }}>
      ...
      {show && (
        <div style={{ background: "#f9f9f9", borderRadius: "8px", padding: "12px 16px", marginTop: "8px", fontSize: "13px", lineHeight: "2" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ color: "#888" }}>Seismic hazard zone</span>
            <span style={{ padding: "2px 8px", borderRadius: "12px", fontSize: "11px", background: risks.seismic_hazard_zone ? "#fce4ec" : "#e8f5e9", color: risks.seismic_hazard_zone ? "#c62828" : "#388e3c" }}>
          ...
          {risks.flood_zone && flood && (
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span style={{ padding: "2px 8px", borderRadius: "12px", fontSize: "11px", background: flood.bg, color: flood.color }}>{flood.level} risk</span>
            </div>
          )}
          {risks.flood_zone_description && <div style={{ color: "#aaa", fontSize: "11px", marginTop: "4px" }}>{risks.flood_zone_description}</div>}
          <div style={{ marginTop: "8px", fontSize: "11px", color: "#aaa" }}>Seismic data from SF DBI · Flood data from FEMA NFHL</div>
        </div>
      )}
    </div>
  );
...
export default function App() {
  ...
  async function handleSearch() {
    ...
    const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchQuery + ", San Francisco, CA")}`);
    ...
    if (mapRef.current) mapRef.current.flyTo([parseFloat(lat), parseFloat(lon)], 16);
    handleMapClick({ lat: parseFloat(lat), lng: parseFloat(lon) });
  }

  async function handleCheckout() {
    const res = await fetch("https://parcel-production-970b.up.railway.app/create-checkout-session", { method: "POST" });
    const data = await res.json();
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100vh", fontFamily: "sans-serif" }}>
        <div style={{ padding: "12px 16px", background: "#fff", borderBottom: "1px solid #ddd", zIndex: 1000 }}>
          ...
          <div style={searchBarStyle.wrapper}>
            <input type="text" placeholder="Search an address in SF..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()} style={searchBarStyle.input} />
        ...
        <div style={{ flex: 1, position: "relative" }}>
          <MapContainer center={[37.7749, -122.4194]} zoom={13} style={{ height: "100%", width: "100%" }} ref={mapRef}>
            ...
            {marker && <Marker position={[marker.lat, marker.lng]}><Popup>{result?.zone_code || "Loading..."}</Popup></Marker>}
            {boundary && <Polygon positions={boundary[0].map(([lng, lat]) => [lat, lng])} pathOptions={{ color: "#2563eb", fillColor: "#2563eb", fillOpacity: 0.15, weight: 2 }} />}
          </MapContainer>
          {!panelOpen && (
            <div style={{ position: "absolute", bottom: "24px", left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.7)", color: "#fff", padding: "10px 18px", borderRadius: "24px", fontSize: "14px", zIndex: 1000, whiteSpace: "nowrap" }}>
              Tap anywhere on the map
            </div>
          )}
          {panelOpen && (
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "#fff", borderRadius: "16px 16px 0 0", boxShadow: "0 -4px 24px rgba(0,0,0,0.15)", padding: "16px", maxHeight: "55vh", overflowY: "auto", zIndex: 1000 }}>
